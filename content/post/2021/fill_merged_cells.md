...
```
Here we are implementing `if (!column %in% names(dat))`. Again we are using `!` to tell R to test the opposite. `column %in% names(dat)` will search for the value of `column` if it is present in the vector `names(dat)` and return `TRUE`, or `FALSE` when it is not present.
